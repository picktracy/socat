...
int _xioopen_ipdgram_listen(struct single *xfd,
		int xioflags, struct sockaddr *us, socklen_t uslen,
		struct opt *opts, int pf, int socktype, int ipproto)
{
	...
	if (retropt_string(opts, OPT_RANGE, &range) >= 0) {
		if (xioparserange(range, pf, &xfd->para.socket.range) < 0) {
			free(range);
			return STAT_NORETRY;
		}
		free(range);
	...
	while (true) {	/* we loop with fork or prohibited packets */
		/*
		   now wait for some packet on this datagram socket,
		   get its sender address, connect there, and return
		 */
		...
		xfd->fd = xiosocket(opts, pf, socktype, ipproto, E_ERROR);
		if (xfd->fd < 0)
		...
		doreuseaddr |=
			(retropt_int(opts, OPT_SO_REUSEADDR, &reuseaddr) >= 0);
		if (doreuseaddr &&
			(Setsockopt(xfd->fd, opt_so_reuseaddr.major,
				opt_so_reuseaddr.minor, &reuseaddr,
						sizeof(reuseaddr))< 0)) {
			Warn6("setsockopt(%d, %d, %d, {%d}, "F_Zd"): %s",
				xfd->fd, opt_so_reuseaddr.major,
				opt_so_reuseaddr.minor, reuseaddr,
				sizeof(reuseaddr), strerror(errno));
		}
		...
		if (Bind(xfd->fd, us, uslen) < 0) {
			Error4("bind(%d, {%s}, "F_socklen"): %s",
				xfd->fd, sockaddr_info(us, uslen, infobuff,
				sizeof(infobuff)), uslen, strerror(errno));
			return STAT_RETRYLATER;
		}
		/*
		   under some circumstances bind() fills
		   sockaddr with interesting info.
		 */
		...
		do {
			result = Recvfrom(xfd->fd, buff1,
					1, MSG_PEEK, &them->soa, &themlen);
		} while (result < 0 && errno == EINTR);
		if (result < 0) {
			Error5("recvfrom(%d, %p, 1, MSG_PEEK, "
				"{%s}, {"F_socklen"}): %s", xfd->fd, buff1,
				sockaddr_info(&them->soa, themlen, infobuff,
				sizeof(infobuff)), themlen, strerror(errno));
			return STAT_RETRYLATER;
		}
		...
		if (xiocheckpeer(xfd, them, la) < 0) {
			Notice1("forbidding UDP connection from %s",
					sockaddr_info(&them->soa, themlen,
						infobuff, sizeof(infobuff)));
			/* drop packet */
			Recv(xfd->fd, buff1, sizeof(buff1), 0);
			Close(xfd->fd);
			continue;
		}
		...
		if (dofork) {
			...
			/*
			   ...
			   when we dont close this we get awkward behaviour
			   on Linux 2.4:
	...
	if (Connect(xfd->fd, &them->soa, themlen) < 0) {
		Error4("connect(%d, {%s}, "F_socklen"): %s",
			xfd->fd, sockaddr_info(&them->soa, themlen,
			infobuff, sizeof(infobuff)), themlen, strerror(errno));
		return STAT_RETRYLATER;
	}
	...
	xiosetsockaddrenv("SOCK", (union sockaddr_union *)us, uslen, ipproto);
	xiosetsockaddrenv("PEER", them, themlen, ipproto);
...
int xioopen_ipdgram_listen(int argc, const char *argv[], struct opt *opts,
   ...
   return _xioopen_ipdgram_listen(&fd->stream,
		xioflags, &us.soa, uslen, opts, pf, socktype, ipproto);
}
